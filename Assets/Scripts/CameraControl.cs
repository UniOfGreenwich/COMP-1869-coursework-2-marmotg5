...
public class CameraControl : MonoBehaviour
{
	...
	void HandleCameraControl()
	{
		if (cameraState == CameraState.CAMERA_LOCKED)
		{
			...
			{
				mainCamera.transform.RotateAround(offsetCoords, Vector3.up, cameraRotatingSpeed * Time.deltaTime);
			}
			else if (Input.GetKey(moveCamRightKey))
			{
				mainCamera.transform.RotateAround(offsetCoords, Vector3.down, cameraRotatingSpeed * Time.deltaTime);
			}
		}
		else if (cameraState == CameraState.CAMERA_FREEMODE)
		{
			...
			//Vector3 inputVector = new Vector3(horizontalInput * cameraMovingSpeed * Time.deltaTime, 0.0f, verticalInput * cameraMovingSpeed * Time.deltaTime);
			...
			if (horizontalInput != 0 || verticalInput != 0)
			{
				...
				//mainCamera.transform.position += cameraDirection * cameraMovingSpeed * Time.deltaTime * currentCameraAcceleration;
			}
			else
			{
				//currentCameraAcceleration -= cameraAcceleration * Time.deltaTime;
